...
THEME_NAME="MaryamOS-Icons-Circle"
...
install() {
  ...
  if [[ "${color}" == '' ]]; then
    ...
    if [[ "${white}" == 'true' ]]; then
      sed --in-place "s/#363636/#dedede/g" "${THEME_DIR}/status"/{16,22,24}/*
    fi
  ...
  if [[ "${color}" == '' && "${theme}" != '' ]]; then
    cp --recursive "${SRC_DIR}/colorful-folder/folder${theme}"/*.svg                                  "${THEME_DIR}/places/48"
  fi

  if [[ "${circle}" == 'true' && "${theme}" == '' && "${color}" == '' ]]; then
    cp --recursive "${SRC_DIR}/circle-folder"/*.svg                                                   "${THEME_DIR}/places/48"
  fi
